...
def feedback1():
    if request.method == 'POST':
        ...
        feedback_text = request.form['feedback']  # Changed 'feedback' to 'feedback_text'
        fname = request.form['fname']
        new_feedback = Feedback(fname=fname, feedback=feedback_text)  # Changed 'feedback' to 'feedback_text'
...
def submit_review():
    if request.method == 'POST':
        ...
        filename = None
        if uploaded_file:
            ext = uploaded_file.filename.split('.')[-1].lower()  # Get the file extension in lowercase
            if ext == 'png':
                filename = f"{cname}_{xname}.jpg"  # Convert PNG to JPG format
                file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)

                # Convert and save the uploaded PNG as JPG
        ...
        # Add the review to the database and commit changes
...
def dashboard(company_name):
    ...
    plt.xticks(ratings)
    plt.tight_layout()
    graph_filename = 'static/response.png'  # Make sure 'static' folder exists
    plt.savefig(graph_filename)
    plt.close()
...
def clogin():
    if request.method == 'POST':
        ...
        if company and check_password_hash(company.password, password):
            # You can add session handling or authentication logic here
